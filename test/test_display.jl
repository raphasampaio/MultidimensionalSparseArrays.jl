...
@testset "Display and Show Methods" begin
    @testset "Empty Array Display" begin
        ...
        @test occursin("NDSparseArray{Float64, 2}", str)
        ...
        @test compact_str == "(3, 3) NDSparseArray{Float64, 2}"
    end
    ...
    @testset "Different Number Types Display" begin
        ...
        str = string(A)
        @test occursin("NDSparseArray{ComplexF64, 2}", str)
        @test occursin("1.0 + 2.0im", str)
        @test occursin("-3.0 - 4.0im", str)

        # Rational numbers
        B = NDSparseArray{Rational{Int}, 3}((2, 2, 2))
        B[1, 1, 1] = 1//2
        B[2, 2, 2] = 3//4

        str = string(B)
        @test occursin("NDSparseArray{Rational{Int64}, 3}", str)
        @test occursin("1//2", str)
        @test occursin("3//4", str)
    end

    @testset "High Dimensional Array Display" begin
        A = NDSparseArray{Float64, 4}((2, 2, 2, 2))
        A[1, 1, 1, 1] = 1.0
        A[2, 2, 2, 2] = 2.0

        str = string(A)
        @test occursin("NDSparseArray{Float64, 4}", str)
    ...
    @testset "Single Dimension Array Display" begin
        ...
        str = string(A)
        @test occursin("NDSparseArray{Int64, 1}", str)
    ...
    @testset "Sparsity Calculation Precision" begin
        ...
        A = NDSparseArray{Int, 2}((3, 4))  # 12 total elements
        A[1, 1] = 1
        A[2, 2] = 2
