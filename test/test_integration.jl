...
@testset "Integration with Julia Array Ecosystem" begin
    ...
    @testset "Iteration Interface" begin
        ...
        values = collect(A)
        expected = [10 0 0; 0 20 0]
        @test values == expected
        
        # Test iteration with enumerate
        for (i, val) in enumerate(A)
            cart_idx = CartesianIndices(A)[i]
            @test val == A[cart_idx]
        end
        
        # Test iteration with zip
        B = similar(A)
        for (a_val, b_idx) in zip(A, CartesianIndices(A))
            B[b_idx] = a_val * 2
        end
        @test B[1, 1] == 20
        @test B[2, 2] == 40
    ...
    @testset "Linear Algebra Compatibility" begin
        ...
        # Test transpose by manual implementation (since transpose might not be implemented)
    ...
    @testset "Conversion and Construction" begin
        ...
        # Test that we can collect back to dense
        dense_reconstructed = collect(sparse)
