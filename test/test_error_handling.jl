...
@testset "Error Handling and Bounds Checking" begin
    ...
    @testset "Linear Indexing Bounds Checking" begin
        A = NDSparseArray{Int, 2}((2, 3))  # 6 elements total
    ...
    @testset "Type Constructor Errors" begin
        ...
        A = NDSparseArray{Int, 2}((1, 3))
        @test size(A) == (1, 3)

        B = NDSparseArray{Int, 1}((5,))
        @test size(B) == (5,)

        # Test dimension count mismatch (this should fail with MethodError)
        @test_throws MethodError NDSparseArray{Int, 2}((3, 3, 3))  # 3 dims for 2D array
        @test_throws MethodError NDSparseArray{Int, 3}((3, 3))     # 2 dims for 3D array
    end
    ...
    @testset "Memory and Resource Errors" begin
        ...
        A = NDSparseArray{Int, 2}((10^6, 10^6))  # Very large but sparse
    ...
    @testset "Edge Case: Zero-Dimensional Arrays" begin
        ...
        A = NDSparseArray{Int, 0}(())
        @test size(A) == ()
        @test ndims(A) == 0

        @test_throws MethodError NDSparseArray{Int, 1}()  # No dimensions provided
    end
    ...
    @testset "Complex Error Scenarios" begin
        ...
        B = NDSparseArray{Complex{Float64}, 2}((3, 3))
        @test_throws DimensionMismatch A + B
